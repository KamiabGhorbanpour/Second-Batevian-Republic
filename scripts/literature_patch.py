...
def unpack(name: str) -> str:
    match = re.search(rf"(?m)^{name} = '([^']+)'$", wrapper)
    if not match:
...
custom = {
    (1, "foreign", 0): "Research on meaningful human control and military-AI governance argues that autonomous systems require clear human responsibility and shared rules. This supports an ending where the Republic gains backing for safeguards while states still disagree over exactly which military uses should be restricted.",
    ...
    (1, "interior", 1): "Legal research on facial recognition and studies of predictive-policing feedback loops stress oversight, justification and limits on data-driven policing. This supports audits and appeals narrowing some uses, while also requiring technical expertise and administrative resources.",
    (1, "interior", 2): "Research shows that predictive policing can create self-reinforcing feedback loops, while field experiments also find that predictive tools can sometimes help allocate police resources. This supports the trade-off in the ending: a ban avoids those feedback risks but also gives up possible efficiency gains.",
    (1, "social", 0): "Experimental and workplace studies find substantial productivity gains from generative AI, while emerging labour-market evidence suggests jobs may be reorganised rather than disappearing uniformly. This supports rapid productivity growth together with fewer routine junior tasks and changing skill demands.",
...
label_pattern = re.compile(r"\b(?:[A-Z]{1,3}\d+(?:\s+and\s+[A-Z]{1,3}\d+)?)\s*\[[^\]]+\]:\s*")
forbidden = (
    ...
    for ministry, decisions in round_data["decisions"].items():
        ...
        for choice_index, choice in enumerate(main_decision["choices"]):
            ...
            if key in custom:
                ...
                overview = choice.get("relevance", "") or ""
                for phrase in forbidden:
                    overview = overview.replace(phrase, "")
                overview = label_pattern.sub("", overview)
                overview = re.sub(r"\s+", " ", overview).strip()
            if not overview:
                raise RuntimeError(f"Missing literature overview for {key}")
            choice["literature_overview"] = overview
            choice["relevance"] = overview
            if key in extra_dois:
                existing = list(choice.get("dois", []) or [])
                for doi in extra_dois[key]:
                    if doi not in existing:
                        existing.append(doi)
                choice["dois"] = existing
...
app, count = pattern.subn(new_citation, app)
if count != 1:
    raise RuntimeError(f"Expected to patch citation_content once, got {count}")
...
for name in ("_MAIN", "_STORY"):
    match = re.search(rf"(?m)^{name} = '([^']+)'$", test)
    if not match:
        raise RuntimeError(f"Missing packed {name} after patch")
    zlib.decompress(base64.b64decode(match.group(1)))

json.loads(
    zlib.decompress(
        base64.b64decode(re.search(r"(?m)^_STORY = '([^']+)'$", test).group(1))
    )
)
print("Patched all main endings with concise literature overviews.")
